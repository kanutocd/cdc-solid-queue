...
The initializer runs inside `Rails.application.config.to_prepare` so reloadable
application constants resolve cleanly, and `CDC_AUTO_CREATE_SLOT=true` maps to
top-level `config.auto_create_slot`.
